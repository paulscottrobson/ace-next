...
static WORD16 palette[8] = { 
	0x000,0x008,0xF00,0xF0F,0x0F0,0x0FF,0xFF0,0xFFF
};
	...
	if (showDisplay) {
		...
		if (showDisplay) {
			...
			for (int x = 0;x < xs;x++) 
			{
				for (int y = 0;y < ys;y++)
			 	{
			 		int ch = CPUReadMemory(0x2400+x+y*32);
			 		int col = HWReadColourRAM(x+y*32);
			 		int xorFlag = (ch & 0x80) ? 0xFF : 0x00;
			 		int xc = x1 + x * 8 * xSize;
			 		int yc = y1 + y * 8 * ySize;
			 		SDL_Rect rc;
					...
					GFXRectangle(&rcf,palette[(col >> 4) & 7]);
		 			...
		 			for (int y = 0;y < 8;y++) {							// 8 Down
		 				rc.x = xc;
		 				rc.y = yc + y * ySize;
		 				int f = CPUReadMemory((ch & 0x7F)*8+0x2C00+y) ^ xorFlag;
